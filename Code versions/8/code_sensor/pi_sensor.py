...
def printPacket(pkt):
    ...
    if ptype == PACKET_TYPE_RESPONSE:
        if cmd == RESP_OK:
            ...
            # The Arduino puts R in params[0], G in params[1], B in params[2]
            r_hz = pkt['params'][0]
            g_hz = pkt['params'][1]
            b_hz = pkt['params'][2]

            print("=" * 30)
            print(f"  COLOR SENSOR READING")
            print(f"  Red:   {r_hz} Hz")
            print(f"  Green: {g_hz} Hz")
            print(f"  Blue:  {b_hz} Hz")
            print("=" * 30)
        else:
...
def handleCameraCommand():
    ...
    # Update counter
    _frames_remaining -= 1
    print(f"Frames remaining: {_frames_remaining}/5")
...
def handleWCommand():
    ...
    sendCommand(COMMAND_BOT_FORWARD)
...
def handleSCommand():
    ...
    sendCommand(COMMAND_BOT_BACKWARD)
...
def handleDCommand():
    ...
    sendCommand(COMMAND_BOT_RIGHT)

def handlePLUSCommand():

    if isEstopActive():
        print("Refused: E-Stop is Active")
        return
    print("Sending + command...")
    sendCommand(COMMAND_CHANGE_MOTOR_SPEED_INCREASE)

def handleMINUSCommand():

    if isEstopActive():
        print("Refused: E-Stop is Active")
        return
    print("Sending - command...")
    sendCommand(COMMAND_CHANGE_MOTOR_SPEED_DECREASE)
...
#continuos key acceptance
import termios
import tty

def getKey():
    """Reads a single keypress from stdin without needing the Enter key."""
    fd = sys.stdin.fileno()
    old_settings = termios.tcgetattr(fd)
    try:
        tty.setraw(sys.stdin.fileno())
        # Use select to check if there is a key waiting so we don't block
...
def runCommandInterface():
    ...
    # Tracking variables for the auto-stop
    last_move_time = 0
    is_moving = False
    MOVE_TIMEOUT = 0.2  # Seconds before robot stops automatically
    ...
    while True:
        ...
        # this is for non conitnuos acceptance
        ...
        # --- 1. Read Keyboard (Instantly) ---
        key = getKey()

        if key:
            char = key.lower()
            if char == '\x03': break  # Handles Ctrl+C to exit
            
            handleUserInput(char)
            
            # If the user pressed a movement key, reset the timer
            if char in ['w', 'a', 's', 'd']:
                is_moving = True
                last_move_time = time.time()

        # --- 2. The Dead Man's Switch logic ---
        # If we were moving, but haven't seen a key recently, stop the robot.
        if is_moving and (time.time() - last_move_time > MOVE_TIMEOUT):
            sendCommand(COMMAND_STOP)
            is_moving = False
            print("Released - Motors Stopped.")
...
if __name__ == '__main__' :
    ...
    try:
        ...
        print("LiDAR NOT CONNECTED. 'l' WONT WORK")
        lidar = None
