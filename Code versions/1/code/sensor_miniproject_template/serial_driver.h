...
bool txEnqueue(const uint8_t *data, uint8_t len) {
    // 1. Calculate current free space in the circular buffer
    // (Total size - elements currently in use)
    uint8_t used = (tx_head >= tx_tail) ? (tx_head - tx_tail) : (TX_BUFFER_SIZE - (tx_tail - tx_head));
    uint8_t free_space = TX_BUFFER_SIZE - used - 1; // -1 to prevent head meeting tail (full)
...
        tx_head = (tx_head + 1) % TX_BUFFER_MASK; // Wrap around if at end
    }

    // 4. Enable the UART Data Register Empty interrupt (UDRIE0)
    // This tells the hardware: "I have data, start sending whenever ready."
    UCSR0B |= (1 << UDRIE0);
...
ISR(USART0_UDRE_vect) {
    if (tx_tail != tx_head) {
        // buffer not empty, send next byte
        UDR0 = tx_buf[tx_tail];
...
bool rxDequeue(uint8_t *data, uint8_t len) {
    // 1. Check if enough bytes are available
    uint8_t head_snapshot = rx_head; // snapshot since ISR can modify rx_head
    uint8_t available = (head_snapshot >= rx_tail) ? (head_snapshot - rx_tail) : (RX_BUF_SIZE - (rx_tail - head_snapshot));
    
    if (len > available) {
        return false;
    }
    
    // 2. Copy data out of buffer
    for (uint8_t i = 0; i < len; i++) {
...
ISR(USART0_RX_vect) {
    // 1. Read the byte from the hardware register immediately
    uint8_t data = UDR0;
    // 2. Calculate the next position for the head (write index)
    uint8_t next_head = (rx_head + 1) % RX_BUFFER_MASK;
    // 3. Check if the buffer is full
    // If the next position is where the tail is, we have no room!
    if (next_head != rx_tail) {
        // 4. Store the byte and update the head
        rx_buf[rx_head] = data;
        rx_head = next_head;
    }
    // Else: Buffer is full, the byte in 'data' is simply discarded
}
