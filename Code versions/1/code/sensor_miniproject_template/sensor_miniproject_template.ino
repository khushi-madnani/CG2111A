...
ISR(INT4_vect){
    ...
    if (interruptTime - lastInterruptTime > 50) {
        bool pinHigh = PINE & 0b00010000;
        if (pinHigh & buttonState == STATE_RUNNING){
            buttonState = STATE_STOPPED;
            stateChanged = true;
        }
        if (!pinHigh & buttonState == STATE_STOPPED){
...
void setup() {
    ...
    // amit: added part for activity 1
    cli();  
    ...
    // ash: added on 13/3 (activity 2)
    DDRA |= (1<<S0_PIN) | (1<<S1_PIN) | (1<<S2_PIN) | (1<<S3_PIN);
    DDRA &= ~(1<<OUT_PIN);
    // ash: set frequency scaling to 20%
    PORTA |= (1<<S0_PIN);
    PORTA &= ~(1<<S1_PIN);
