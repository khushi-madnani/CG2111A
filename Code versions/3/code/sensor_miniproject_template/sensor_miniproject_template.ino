...
ISR(INT4_vect){
    uint32_t interruptTime = millis(); // Use uint32_t for time
    
    if (interruptTime - lastInterruptTime > 100) {
        bool isPressed = (PINE & (1 << PE4));
        
        // Determine what the state SHOULD be
        TState targetState = isPressed ? STATE_STOPPED : STATE_RUNNING;

        // If current state doesn't match the physical button, fix it
    ...
    // int32_t interruptTime = millis();
    // if (interruptTime - lastInterruptTime > 75) {
    //     bool isPressed = (PINE & (1 << PE4));
    //     lastInterruptTime = interruptTime;
