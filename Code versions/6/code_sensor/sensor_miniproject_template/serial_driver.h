...
bool txEnqueue(const uint8_t *data, uint8_t len) {
    ...
    uint8_t free_space = TX_BUFFER_SIZE - used - 1; 
    ...
    for (uint8_t i = 0; i < len; i++) {
        ...
        tx_head = (tx_head + 1) & TX_BUFFER_MASK; // Use MASK for power-of-2 wrap
    }
...
bool rxDequeue(uint8_t *data, uint8_t len) {
    uint8_t head_snapshot = rx_head; 
    uint8_t available = (head_snapshot >= rx_tail) ? (head_snapshot - rx_tail) : (RX_BUFFER_SIZE - (rx_tail - head_snapshot));
